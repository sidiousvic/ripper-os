...
export function parseTrainingFile(fileBytes: Uint8Array, fileName: string) {
  ...
  const xlsx = fileName.toLowerCase().endsWith(".xlsx");
    ...
    const achievements = exercises.filter((x) => x.progress.length > 1).slice(0, 4).map((x) => { const firstPoint = x.progress[0], latest = x.progress.at(-1), value = (p: any) => r(n(p[x.defaultMetric])); const peak = x.progress.reduce((best, point) => value(point) > value(best) ? point : best, firstPoint); return { exercise: x.name, metric: x.defaultMetric, first: { date: firstPoint.date, value: value(firstPoint) }, latest: { date: latest.date, value: value(latest) }, peak: { date: peak.date, value: value(peak) }, percentChange: value(firstPoint) ? r((value(latest) / value(firstPoint) - 1) * 100, 0) : 0 }; });
