...
import { sha256Bytes } from "../lib/import/file-identity.ts";
...
const { createImportPreview } = await import('../lib/import/import-preview.ts');
const { createImportReport } = await import('../lib/import/import-report.ts');
const strongPreview = createImportPreview(parsedStrong, "strong.csv", "replace", []);
assert.equal("conflict" in strongPreview, false);
assert.equal(strongPreview.trainingDays, 86);
assert.equal(strongPreview.knownWorkouts, 86);
assert.equal(strongPreview.nextImports.length, 1);
const strongReport = createImportReport(strongPreview);
assert.equal(strongReport.addedRecords, 86);
assert.ok(!strongReport.diagnostics.includes('original-export.csv'));
assert.ok(!strongReport.diagnostics.includes('Bench Press'));
const strongHash = await sha256Bytes(strongFixtureBytes);
assert.equal(strongHash.length, 64);
const hashedStrong = { ...parsedStrong, importData: { ...parsedStrong.importData, contentHash: strongHash } };
const duplicatePreview = createImportPreview(hashedStrong, "renamed-strong.csv", "add", createImportPreview(hashedStrong, "strong.csv", "replace", []).nextImports);
assert.equal("conflict" in duplicatePreview, false);
...
const { combineImports } = await import('../lib/history/combine-imports.ts');
const mfBenchInput = 'Date,Exercise,Weight (kg),Reps\n2019-01-02,Barbell Bench Press,80,5\n';
const mfBenchImport = normalizeMacroFactor(inspectInput(new TextEncoder().encode(mfBenchInput), 'mf.csv'), 'mf.csv');
const comparableStrong = normalizeStrong(strongInput, 'strong.csv', { weightUnit: 'kg', distanceUnit: 'km', loadSemantics: { 'Bench Press (Barbell)': { component: 'external', basis: 'total' } } });
assert.equal(comparableStrong.status, 'ready');
const combined = combineImports([mfBenchImport], comparableStrong.data);
assert.equal(combined.ok, true);
const combinedDashboard = buildDashboard(combined.imports);
const combinedBench = combinedDashboard.exercises.find(exercise => exercise.exerciseId === 'barbell_bench_press');
assert.ok(combinedBench);
assert.equal(combinedBench.sessions, 3);
assert.equal(combinedBench.progress.length, 3);
assert.deepEqual(combinedBench.progress.map(point => point.date), ['2019-01-02', '2021-06-11', '2021-09-02']);
const originalImportCount = mfBenchImport.exerciseDays.length;
const overlap = combineImports([mfBenchImport], { ...mfBenchImport, importId: 'mf:duplicate', exerciseDays: mfBenchImport.exerciseDays.map(day => ({ ...day, importId: 'mf:duplicate' })) });
assert.equal(overlap.ok, true);
assert.equal(overlap.status, 'unchanged');
assert.equal(mfBenchImport.exerciseDays.length, originalImportCount);
assert.equal(buildDashboard([mfBenchImport]).coverage.totalSessions, 1);
console.log('History combiner: disjoint MF/Strong bench series and unchanged re-export passed');
await import('./test-history-reconciliation.mjs');
