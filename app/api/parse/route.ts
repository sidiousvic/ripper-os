...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!wb.SheetNames.length || wb.SheetNames.length > 20) throw new Error("Unsupported workbook structure.");
    ...
    if (file.name.toLowerCase().endsWith(".csv")) {
      ...
      const dateCol = index("Date"), exerciseCol = index("Exercise"), weightCol = index("Weight (kg)"), repsCol = index("Reps"), durationCol = index("Workout Duration");
