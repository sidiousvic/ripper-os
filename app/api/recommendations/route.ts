...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ sustainedPractice: text(parsed.sustainedPractice, 240, "Your consistency story will appear here after analysis."), nextYearRule: text(parsed.nextYearRule, 240, "Choose a small, repeatable change and measure it consistently."), recommendations, generatedAt: new Date().toISOString(), model: process.env.OPENAI_MODEL ?? "gpt-5-mini" }, { headers: { "cache-control": "no-store" } });
  } catch (error) {
