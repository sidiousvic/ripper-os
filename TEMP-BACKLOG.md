...
| 6 | Before sharing | Review remaining development-tool dependency advisories. | Complete in v0.2.13. Updated the Cloudflare/Vinext/Vite/Wrangler toolchain, React server packages, and Workers types; full and production audits now report zero vulnerabilities. |
| 7 | Beta UX | Keep About directly accessible before upload and at every screen width. | Complete in v0.2.14. A standalone About link remains visible when the dashboard navigation is hidden on the empty state and at responsive widths. |
| 8 | Beta UX | Improve modal keyboard access, loading/cancellation, and optional-AI onboarding. | Complete in v0.2.15. Dialogs close on Escape, trap Tab focus, and restore focus. AI generation has an explicit cancellation action; AI remains optional and the core dashboard remains usable without it. |
