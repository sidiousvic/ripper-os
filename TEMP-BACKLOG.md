...
| 7 | Before sharing | Make snapshot storage/restore safe and independent of successful uploads. | Complete in v0.2.7. Storage failures remain independent of rendering; restore now validates the saved dashboard schema and removes malformed snapshots. Quota retry, stale snapshot removal, size limits, and invalid-snapshot checks are covered. |
