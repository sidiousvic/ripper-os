...
| 1a | Next: user priority | Process files and calculate summaries in a browser Web Worker; remove the server upload endpoint. | Pending. Workbook must stay on the device; only explicitly requested AI summaries may reach the server. Supersedes the server-upload architecture in items 3, 6, and part of 9. |
...
| 3 | Before sharing | Align upload limits with Vercel and handle oversized/non-JSON responses gracefully. | Pending. UI promises 25 MB; Vercel request limit is 4.5 MB. |
...
| 6 | Before sharing | Correct privacy and persistence copy. | Pending. Raw files reach the server for parsing; only summaries go onward to OpenAI. |
...
| 8 | Before sharing | Prevent stale upload and AI responses from updating another dataset. | Pending. Add cancellation or request/dataset identity checks; verify clear/replace during pending requests. |
| 9 | Before sharing | Handle malformed multipart requests and bound parser work. | Pending. Malformed content type produced an unhandled 500; review date, expansion, and processing limits. |
